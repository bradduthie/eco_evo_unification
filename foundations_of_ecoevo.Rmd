...
NOTE: ADD CASE AND TAPER (2000) AS A REFERENCE FOR Delta N = N bar(w)
...
Over an arbitrary length of time, fitness is properly defined as $w_{i} = \beta_{i} - \delta_{i} + 1$. <!--- For simplicity, we defined beta and delta as births and deaths, but we could more generally define beta as the count of descendents of i, and delta as the count of deaths in i and all of its descendents --->
...
Fundamentally, we show why eco-evolutionary change is a statistical interaction between fitness and an individual characteristic.
From this definition, we can recover both population size change and evolutionary change.
Our $\Omega$ can be therefore be interpreted as characteristic-scaled population change, with the special case of $z = 1$ recovering population size change by recognising that each individual contributes one to the total.
...
Lastly, we thank two anonymous reviewers.
