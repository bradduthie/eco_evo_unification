...
In other words, we set $z_{i} = 1$ to simply indicate that $i$ is one member of the population. <!--- and therefore contributes one unit toward the total population size. --->
In this restricted case, $z$ is count, which takes the unit 1 [note that 'individual' is a label, not a unit, see @NewellEtAl2019].
Further, we assume that individuals do not change species by setting $\Delta z_{i} = 0$. <!--- Further, we assume that individual membership and contribution to population size does not change by... --->
In this case,
...
Summing from 1 to $N_{t}$, we can rewrite the above,

$$N_{t+1} = N_{t}\left(1 + \beta - \delta \right).$$
