...
The deep interdependence between ecology and evolution is well-established, and recent models have focused on integrating eco-evolutionary dynamics. 
Nevertheless, a gap remains between the logical foundations of ecology and evolution. 
We bridge this gap with an equation that defines a summed value for a characteristic across individuals in a population, from which the fundamental equations of population ecology and evolutionary biology (the Price equation) are derived. 
We thereby identify an overlooked bridge between the fundamental equations of population ecology and biological evolution. 
...
We outline how our proposed framework can be used to further develop eco-evolutionary theory. 
...
Our aim here is to propose an equation that extends this conceptual clarity to include population change and thereby provide a formal and exact definition for joint ecological and evolutionary change.
...
Derivation follows by adding assumptions that are specific to population ecology or evolution in the same way that key equations of population genetics (e.g., the average excess) or quantitative genetics (e.g., the Breeder's equation) can be derived from the Price equation [@Queller2017].
...
This foundation of eco-evolutionary change is an interaction between the demographic processes of birth and death ($\beta_{i} - \delta_{i} + 1$) and some characteristic of an individual ($z_{i} + \Delta z_{i}$).
...
In this case, $z$ is count, which takes the unit 1 [note that 'individual' is a label, not a unit, see @NewellEtAl2019].
...
Summing up $\beta_{i}$, $\delta_{i}$, and current entities tallies up the total number of entities in the next time step,
...
In such cases, the sum across individuals gives a meaningful total quantity for the population.
...
We hope that this will be useful for eco-evolutionary theory in a similar way that the Price equation is useful for evolutionary theory: potentially facilitating specific model development and identifying new conceptual insights, unresolved errors, and sources of model disagreements.
