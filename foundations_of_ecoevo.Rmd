...
From this foundation, we can define $\Omega$ to be the sum of character values in $t+1$,
...
There's an odd technicality if the time between t and t + 1 is large. I defined beta as 'births attributable to i', which is correct, but maybe it would be more correct to define it as 'descendents of i'. Then, delta needs to be more technically 'deaths of i and its descendents'). Nothing changes in effect, but now if a focal individual i has a bunch of descendents that die between t and t + 1, the correct fitness is still preserved.

Put this after the sentence about Omega?
...
The value of $\Omega$ takes the same units as $z$.
